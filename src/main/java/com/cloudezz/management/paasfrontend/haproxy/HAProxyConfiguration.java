...
import com.cloudezz.management.paasfrontend.haproxy.impl.HAProxyRegistry;

/**
 * Class defining the HAProxy load balancer configuration.<br>
 * The default configuration settings and global parameters are defined in a configuration file (see setDefaults)<br>
 * <br>
 * Currently 2 front ends/proxy are supported (HTTP and HTTPS). An application can support either or both.<br>
 * <br>
 * Application stickyness is supported via client source IP and HTTP cookie. An application can support either or both.<br>
 * In case where both are defined, cookie is the preferred mechanism and IP source the fall back mechanism if cookies are
 * not supported by the client.<br>
 * - client IP tracking: Default is 1000 entries with 30 minutes expiration. This can be changed
 * via PaaSApplication.setIPStickynessOption<br>
 * - cookie tracking: The cookie name is SERVERID<br>
 * <br>
 * Server node error can be redirected to a URL. The redirect uses HTTP status 303 to handle non GET requests<br>
 *<br>
 * The load balancing algorithm is roundrobin<br>
 *<br>
 * Server node health check are by default performed by establishing a TCP connection. A full HTTP request can be used see PaaSApplication.setNodeHealthCheckOption<br>
 *<br>
 * URL request rewrite is added if URL path is defined: for example request to "company.domain.com/request?param=x" is rewritten to
 * "company.domain.com/NODEPATH/request?param=x" where NODEPATH is the url path defined in the PaasNode.url<br>
 *<br>
 * Frontend/proxy rules for the selection of the appropriate backend is based on matching the PaaSApplication name to the HTTP request source domain URL<br>
 * 
 */
public class HAProxyConfiguration {
    ...
    private void setDefaults(InputStream confFile) throws IOException {
        ...
        while ((s = br.readLine()) != null) {
            ...
            if (s.equalsIgnoreCase(GLOBAL_SETTINGS)) {
                ...
                Proxy frontEnd = addFrontEnd(name, connection);
    ...
    /**
     * The load balancer default configuration parameters and defined in a specified file. The load balancer configuration
     * type is HAProxy.
    ...
    /**
     * This method set an application load balancing configuration.  Two front ends are currently supported (HTTP and HTTPS)
     * An application can support either or both.
    ...
    public void addApplication(PaaSApplication app) throws MalformedURLException {
        ...
        // Add client IP tracking. Default is 1000 entries with 30 minutes expiration. This can be changed
        ...
        // Current strategy is transparent to server meaning that the cookie is inserted by the proxy and is
